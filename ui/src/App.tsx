...
function App() {
    ...
    const windowOptions = ["1m", "5m", "10m", "30m", "1h", "3h", "6h", "12h", "24h"]
    ...
    const chartData = useMemo(() => {
        ...
        seriesData.forEach((series) => {
            ...
            for (const p of series.plans) {
                for (const cpu of p.cpu_time_millis) {
        ...
        for (const digest of orderedDigests) {
            const seriesRecord: Record<number, number> = {};
            const rawSQLSeries = valuesByDigest[digest];

            for (const rawSeries of rawSQLSeries.plans) {
                const len = rawSeries.cpu_time_millis.length;
                for (let i = 0; i < len; i++) {
                    const ts = rawSeries.timestamp_secs[i] * 1000;
                    if (!(ts in seriesRecord)) {
                        seriesRecord[ts] = 0;
                    }
                    seriesRecord[ts] += rawSeries.cpu_time_millis[i];
                }
            }

            const series: Array<[number, number]> = [];
            Object.entries(seriesRecord).forEach(([ts, cpu]) => {
                series.push([+ts, cpu]);
            });

            data.push({
                Label: {
                    SQLDigest: rawSQLSeries.sql_digest,
                    SQLText: rawSQLSeries.sql_text
                },
                Series: series,
            });
        }
